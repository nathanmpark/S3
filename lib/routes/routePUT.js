...
function _pushMetrics(err, utapi, action, resource, contentLength,
    ...
    if (!err) {
        ...
        if (action === 'bucketPutACL') {
            utapi.pushMetricPutBucketAcl(resource, timestamp);
        } else if (action === 'bucketPut') {
            utapi.pushMetricCreateBucket(resource, timestamp);
        } else if (action === 'objectPutPart') {
            utapi.pushMetricUploadPart(resource, timestamp, contentLength);
        } else if (action === 'objectPutACL') {
            utapi.pushMetricPutObjectAcl(resource, timestamp);
        } else if (action === 'objectPut') {
            utapi.pushMetricPutObject(resource, timestamp, contentLength,
                prevContentLen);
        }
    }
}
    ...
    if (request.objectKey === undefined || request.objectKey === '/') {
        ...
        request.on('end', () => {
            ...
            if (request.query.acl !== undefined) {
                api.callApiMethod('bucketPutACL', request, log, err => {
                    _pushMetrics(err, utapi, 'bucketPutACL',
                        request.bucketName);
        ...
        if (request.query.partNumber) {
            api.callApiMethod('objectPutPart', request, log, err => {
                ...
                _pushMetrics(err, utapi, 'objectPutPart', request.bucketName,
            ...
            request.on('end', () => {
                api.callApiMethod('objectPutACL', request, log, err => {
                    _pushMetrics(err, utapi, 'objectPutACL',
                        request.bucketName);
                    return routesUtils.responseNoBody(err, null, response, 200,
                        log);
                });
            ...
            api.callApiMethod('objectPut', request, log,
                (err, contentMD5, prevContentLen) => {
                    ...
                    _pushMetrics(err, utapi, 'objectPut', request.bucketName,
                        request.headers['content-length'], prevContentLen);
